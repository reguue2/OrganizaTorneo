...
function getEmailConfig() {
  ...
  return {
    provider: env.registrationEmailProvider,
    resendApiKey: env.resendApiKey ?? "",
    from: env.registrationEmailFrom,
    replyTo: env.registrationEmailReplyTo,
    supportEmail: env.registrationSupportEmail,
  }
}
...
export function renderRegistrationVerificationEmail(
  ...
  const expiresAt = new Date(payload.expiresAt).toLocaleString("es-ES")
...
export async function dispatchRegistrationVerificationEmail(
  ...
  const email = renderRegistrationVerificationEmail(payload)
  ...
  const paymentLabel =
    payload.paymentMethod === "cash"
      ...
      : payload.paymentMethod === "online"
        ? "Online"
        : "—"
  const categoryLine = payload.categoryName
    ? `<p><strong>Categoría:</strong> ${escapeHtml(payload.categoryName)}</p>`
    : ""
  const cancelCodeLine = payload.cancelCode
    ? `<p><strong>Código de cancelación:</strong> ${escapeHtml(payload.cancelCode)}</p>`
    : ""
  const cancelUrlLine = payload.cancelUrl
    ? `<p><strong>Enlace de cancelación:</strong> <a href="${escapeHtml(
        payload.cancelUrl
      )}">${escapeHtml(payload.cancelUrl)}</a></p>`
    : ""
  ...
  return {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; line-height: 1.5; color: #111827;">
        ...
        <p><strong>Torneo:</strong> ${escapeHtml(payload.tournamentTitle)}</p>
        ${categoryLine}
        <p><strong>Referencia pública:</strong> ${escapeHtml(payload.publicReference)}</p>
        <p><strong>Estado:</strong> ${escapeHtml(getRegistrationStatusLabel(payload.registrationStatus))}</p>
        <p><strong>Canal de pago:</strong> ${escapeHtml(paymentLabel)}</p>
        <p><strong>Importe:</strong> ${escapeHtml(formatMoney(payload.amount))}</p>
        ${cancelCodeLine}
        ${cancelUrlLine}
...
export async function dispatchRegistrationConfirmationEmail(
  ...
  const email = renderRegistrationConfirmationEmail(payload)
