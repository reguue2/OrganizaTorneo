...
-- Initial Supabase baseline for Organiza Torneo.
-- Development-only data can be reset; this file is the current schema source of truth.
...
begin
  ...
  from public.tournaments t
  where r.tournament_id = t.id
    and r.status = 'pending_cash_validation'::public.registration_status
    and (
      t.status in (
        'finished'::public.tournament_status,
        'cancelled'::public.tournament_status
...
begin
  ...
  if v_registration.status <> 'pending_cash_validation'::public.registration_status then
...
begin
  ...
  if p_has_categories then
    ...
    if p_categories is null
       ...
       or jsonb_array_length(p_categories) < 1 then
...
CREATE TABLE IF NOT EXISTS "public"."registrations" (
    "id" "uuid" DEFAULT "extensions"."uuid_generate_v4"() NOT NULL,
    "category_id" "uuid",
    "status" "public"."registration_status" DEFAULT 'pending_cash_validation'::"public"."registration_status",
    "payment_method" "public"."registration_payment_method",
    "created_at" timestamp with time zone DEFAULT "now"(),
...
CREATE POLICY "Authenticated users can delete own tournament posters" ON "storage"."objects" FOR DELETE TO "authenticated" USING ((("bucket_id" = 'tournament-posters'::text) AND (("storage"."foldername"("name"))[1] = ("auth"."uid"())::text)));



CREATE POLICY "Authenticated users can update own tournament posters" ON "storage"."objects" FOR UPDATE TO "authenticated" USING ((("bucket_id" = 'tournament-posters'::text) AND (("storage"."foldername"("name"))[1] = ("auth"."uid"())::text))) WITH CHECK ((("bucket_id" = 'tournament-posters'::text) AND (("storage"."foldername"("name"))[1] = ("auth"."uid"())::text)));



CREATE POLICY "Authenticated users can upload own tournament posters" ON "storage"."objects" FOR INSERT TO "authenticated" WITH CHECK ((("bucket_id" = 'tournament-posters'::text) AND (("storage"."foldername"("name"))[1] = ("auth"."uid"())::text)));



CREATE POLICY "Public can read tournament posters" ON "storage"."objects" FOR SELECT TO "authenticated", "anon" USING (("bucket_id" = 'tournament-posters'::text));
...
-- Security baseline: keep direct table access narrow and expose only the RPCs
-- each client role actually needs.
...
GRANT EXECUTE ON FUNCTION "public"."cancel_public_registration"("text", "text", "text") TO "anon", "authenticated";
...
GRANT EXECUTE ON FUNCTION "public"."approve_cash_registration"("uuid") TO "authenticated";
GRANT EXECUTE ON FUNCTION "public"."cancel_registration_by_organizer"("uuid") TO "authenticated";
GRANT EXECUTE ON FUNCTION "public"."mark_online_registration_paid"("uuid") TO "authenticated";
GRANT EXECUTE ON FUNCTION "public"."publish_tournament"("uuid") TO "authenticated";
GRANT EXECUTE ON FUNCTION "public"."set_tournament_management_status"("uuid", "public"."tournament_status") TO "authenticated";
